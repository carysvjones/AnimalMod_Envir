...
gtit_data_sub_nb <- gtit_data_sub_nb %>%
  group_by(Mother) %>%
  mutate(lay_hatch_lag = April.hatch.date - April.lay.date) %>%
  ungroup()

nrow(subset(gtit_data_sub_nb, !is.na(April.lay.date) &
  !is.na(April.hatch.date))) # 10893

# correlation coefficient r2
cor(gtit_data_sub_nb$April.lay.date, gtit_data_sub_nb$April.hatch.date,
  method = "pearson", use = "complete.obs"
) #0.952
summary(lm(April.hatch.date ~ April.lay.date, data = gtit_data_sub_nb))
# r2 of 0.906
...
# model basic
LD_basic <- asreml(
  fixed = April.lay.date ~ 1 + Fbreeding_age_group,
...
HD_basic <- asreml::asreml(
  fixed = April.hatch.date ~ 1 + Fbreeding_age_group,
  random = ~ vm(Mother, ped_data_inv) +
    ide(Mother, ped_data_inv) +
    breeding_year,
  data = breed_data
