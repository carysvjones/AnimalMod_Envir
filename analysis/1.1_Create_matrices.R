...
dim(spat_mat_conv) <- c(nrow(spatial_geom),
                        nrow(spatial_geom)) #give it dimensions?
...
# env_matrix_scal <- abs(1 - env_matrix)
env_matrix <- round(env_matrix[1:5000,1:5000], digits=3)
# env_matrix_scal <- round(env_matrix_scal[1:10,1:10], digits=3)
...
spatial_raw_df <- as.data.frame(as.table(spat_matrix_raw))
...
#merge
spat_env_df <- left_join(spatial_raw_df, env_df, by = c('Mother1', 'Mother2'))

spat_env_df <- merge(spatial_raw_df, env_df, by = c('Mother1', 'Mother2'))
nrow(spat_env_df) #9900
...
envir_sim_hist <-
  ...
  geom_histogram(aes(y = ..density..),
                 ...
                 colour = '#2F5597',
                 alpha = 0.7) +
  geom_density(fill = "#77AADE",
               alpha = 0.8) +
  geom_vline(xintercept = median(spat_env_df$env_sim),
             colour = '#2F5597',
             linetype = 'dashed',
             linewidth = 0.7) +
  xlab('Environmental similarity measure') + ylab('Count') +
  output_plot_theme
...
comp_mat <- ggplot(spat_env_df, aes(x = dist, y = env_sim) ) +
  ...
  theme(panel.background = element_rect(fill = "#B4C7E7", color = "#B4C7E7"),
...
ggsave(
  filename = file.path(dirs$plots, "compare_matrices2.png"),
  plot = comp_mat_opp,
  scale = 1,
  width = 26,
  height = 22,
