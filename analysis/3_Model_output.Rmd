...
env_var_plot <-
  ggplot(
    subset(LD_varcomps_mat, name %in% c("Vnb", "Vspat", "Venv")),
    aes(
      x = model_name_plot,
      y = Rel_Est
    )
  ...
  geom_hline(
    yintercept = c(0.0375, 0.0625, 0.0875, 0.1125, 0.1375, 0.1625, 0.1875, 0.2125, 0.2375, 0.2625),
    linetype = "dashed",
    color = "light grey",
    linewidth = 0.15
  ) +
...
variance_space <- ggplot(
  transform(LD_space, model_simp = factor(model_simp, levels = c("4", "3", "2"))),
  ...
  ylab("") +
  xlab("Proportion within-year variance (SE)") +
  theme(
