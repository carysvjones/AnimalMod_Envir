...
nrow(ped_gtit_short_ext_fix) # 98519
...
breed_gtit_noNA <- breed_gtit_noNA %>%
  ...
  dplyr::select(
    "year", "pnum", "Section", "April.lay.date", "April.hatch.date",
    "Clutch.size", "Num.fledglings", "father", "mother", "nest_box"
  )
...
gtit_data_sub_nb_multisumm <- gtit_data_sub_nb %>%
  ...
  dplyr::summarise(.,
    nest_boxes = length(unique(nest_box)),
    breed_attempts = length(nest_box)
  )
...
multi_box <- gtit_data_sub_nb %>%
  dplyr::group_by(., mother) %>%
  dplyr::filter(length((nest_box)) > 1) %>%
  dplyr::mutate(
    x2 = dplyr::lead(x),
    y2 = dplyr::lead(y),
  ...
  dplyr::ungroup() %>%
  dplyr::select(
    mother, breeding_year, breeding_year2, x, y, x2, y2, nest_box, nest_box2, dist,
