...
write.csv(ped_gtit_short_ext_fix,
  file =
    file.path(dirs$data_output, "GTIT-pedigree-full-fixed.csv"), row.names = F
)
...
missing <- ped_data[!(ped_data$id %in% ped_gtit_final$id), ]
nrow(missing) # 721

missing[missing$id %in% ped_gtit_short$id, ]
# VZ31433
```
...
breed_gtit_noNA <- breed_gtit %>%
  dplyr::filter(is.na(breed_gtit$experiment_codes) &
    !is.na(breed_gtit$april_lay_date))

nrow(breed_gtit_noNA) # 15436
# how many have no mother
nrow(subset(breed_gtit_noNA, is.na(mother))) # 3288
nrow(subset(breed_gtit_noNA, is.na(father))) # 5789

# have both parents
nrow(subset(breed_gtit_noNA, !is.na(mother) & !is.na(father))) # 9260
# have just mother
nrow(subset(breed_gtit_noNA, !is.na(mother))) # 12148
nrow(subset(breed_gtit_noNA, !is.na(mother) & is.na(father))) # 2888 have no mother but have father
# have jsut father
nrow(subset(breed_gtit_noNA, !is.na(father))) # 9647
nrow(subset(breed_gtit_noNA, !is.na(father) & is.na(mother))) # 387 have no father but have mother
...
length(unique(breed_gtit_noNA$mother)) # 7791
length(unique(breed_gtit_noNA$father)) # 6342
length(unique(breed_gtit_noNA$pnum)) # 12148
summary(breed_gtit_noNA$april_lay_date) #-5 to 83


nrow(breed_gtit_noNA) # 12148
...
#    1    2    3    4    5    6    7    8    9
# 6558 3191 1421  622  260   67   23    5    1
...
breed_gtit_noNA_age_sub <- clean$remove_late_broods(breed_gtit_noNA_age,
  mothers = T, fathers = T,
...
summary(as.factor(breed_gtit_noNA_age_sub$Fbreeding_age_group)) # Adult 5368, Yearling 6375
...
# output data
write.csv(gtit_data_sub_nb, file = file.path(dirs$data_output, "GTIT_data_anim_mod2.csv"), row.names = F)
```
